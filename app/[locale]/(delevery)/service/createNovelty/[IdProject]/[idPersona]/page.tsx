...
async function page({ params }) {
  ...
  const listaNovedades = await fetchData(`${novedadWithEntetiesApiUrl}`).then(
    (novedad) => {
      const filter = novedad.filter(
        (item) => item.idPersona == params.idPersona
      );
      ...
      const updatedData = filter.map((item) => ({
        ...
        fechaHasta: item.fechaHasta
          ? new Date(item.fechaHasta).toLocaleDateString('es-CL')
          : 'N/A',
        fechaInicio:
          new Date(item.fechaInicio).toLocaleDateString('es-CL') || 'N/A',
