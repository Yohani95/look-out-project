...
import { updateRegistroHorasProyectoDesarrollo } from '@/app/actions/proyectoDesarrollo/RegistroHorasProyectoDesarrolloActions';
...
const RegistroHorasProyectoDesarrolloEdit: React.FC<FormProps> = ({
  t,
  data,
  id,
}) => {
  ...
  const formik = useFormik({
    initialValues: new RegistroHorasProyectoDesarrollo(data.registroHora),
    validationSchema,
    onSubmit: async (values, { setSubmitting }) => {
      try {
        await Utils.showLoadingNotification(t);
        const plainValues = JSON.parse(JSON.stringify(values));
        delete plainValues.profesionalProyecto;
        await updateRegistroHorasProyectoDesarrollo(plainValues, plainValues.id)
          .then((res) => {
            if (res != 204) {
...
export default RegistroHorasProyectoDesarrolloEdit;
